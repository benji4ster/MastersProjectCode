...
class LeCroy_Scope:
    ...
    #-------------------------------------------------------------------------
    # THIS SCREEN DUMP FUNCTION DOES NOT WORK
    ...
    def timebase(self, trace):
        """Return timebase s/div"""
        trace = self.validate_trace(trace)
        self.scope.write(trace+':WAVEFORM?')
        self.trace_bytes = self.scope.read_raw()
        self.hdr = WAVEDESC._make(struct.unpack(WAVEDESC_FMT, self.trace_bytes[15:15+WAVEDESC_SIZE]))
        return TIMEBASE_IDS[self.hdr.timebase]
    ...
    def settime(self):
        """ Set the scope time to match the computer """
        self.scope.write('VBS app.Utility.DateTimeSetup.Month='+time.strftime("%m",time.localtime()))
        self.scope.write('VBS app.Utility.DateTimeSetup.Validate')
        self.scope.write('VBS app.Utility.DateTimeSetup.Day='+time.strftime("%d",time.localtime()))
        self.scope.write('VBS app.Utility.DateTimeSetup.Validate')
        self.scope.write('VBS app.Utility.DateTimeSetup.Hour='+time.strftime("%H",time.localtime()))
        self.scope.write('VBS app.Utility.DateTimeSetup.Validate')
        self.scope.write('VBS app.Utility.DateTimeSetup.Minute='+time.strftime("%M",time.localtime()))
        self.scope.write('VBS app.Utility.DateTimeSetup.Validate')
        self.scope.write('VBS app.Utility.DateTimeSetup.Second='+time.strftime("%S",time.localtime()))
        # self.scope.write('DATE '+str(time.time()))
        self.scope.write('VBS app.Utility.DateTimeSetup.Validate')

###############################################################################

# scope_ip_addr = '192.168.1.4'   # '128.97.13.149'
...
#             #scope.autoscale('C3')
#             #time.sleep(.5)
#             print(scope.displayed_channels())
