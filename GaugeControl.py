...
def Acquire_Scope_Data_2D(get_hdf5_filename, get_channel_description):
    """
    ...
    with LeCroy_Scope(ip_address, verbose=False) as scope:
...
# scope = LeCroy_Scope(scope_ip_addr,verbose=False)

n = 10 # amount of time (0.1s) between pressure reaadouts
m = 20 # amount of time (s) between scope readings

### Set the time on the scope to match the computer
# with LeCroy_Scope(scope_ip_addr, verbose=False) as scope:
#     scope.settime()

with open(str('data\measurement_data_'+datetime.now().strftime("%m%d")+'.txt'), 'a') as logfile:    
    ...
    while True:
        try:
            ...
            # line += "%.3E, %.3E" % (ps[1].pressure, ps[2].pressure)
            pres = "%.3E" % (ps[2].pressure)        
            # print("Time: ", datetime.now().strftime("%H:%M:%S"))
            print("Pressure: ", pres) 
            
            # Acquire_Scope_Data_2D(get_hdf5_filename, get_channel_description)

            # trace = scope.acquire('C2')
    
            sys.stdout.flush()
            logfile.write(t+pres+'\n')
            logfile.flush()
